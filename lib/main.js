...
class Main {
  constructor(socket) {
    ...
    socket.on("db init", (data) => {
      this.jsondb[data.name] = new jsondb(`${sha256.sync(data.name)}.jsondb`, {
        tableNames: {
          names: [],
        },
      });
  ...
  update_query(data) {
    ...
    for (var i = 0; i < data.columnNames.length; i++) {
      if (typeof data.data[i] == "string") {
        statements += `${data.columnNames[i]} = '${data.data[i]}', `;
      } else {
        statements += `${data.columnNames[i]} = ${data.data[i]}, `;
      }
    }
    ...
    for (var i = 0; i < data.columnNames.length; i++) {
      if (typeof data.default[i] == "string") {
        def += `${data.columnNames[i]} = '${data.default[i]}' AND `;
      } else {
        def += `${data.columnNames[i]} = ${data.default[i]} AND `;
      }
    }
    ...
    statements = `UPDATE ${data.name} SET ${statements} WHERE ${def}`;
